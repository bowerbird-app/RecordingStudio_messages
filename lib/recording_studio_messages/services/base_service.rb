  ...
  module Services
    ...
    class BaseService
      ...
      def run_before_hooks
        ...
        RecordingStudioMessages::Hooks.run(:before_service, self.class, service_args)
      end
      ...
      def run_after_hooks(result)
        ...
        RecordingStudioMessages::Hooks.run(:after_service, self.class, result)
      end
      ...
      def run_with_around_hooks
        if hooks_enabled? && RecordingStudioMessages.configuration.hooks.registered?(:around_service)
          RecordingStudioMessages::Hooks.run_around(:around_service, self) { perform }
        else
          perform
        end
      end

      # Check if hooks are enabled (RecordingStudioMessages is loaded and configured)
      #
      # @return [Boolean]
      def hooks_enabled?
        defined?(RecordingStudioMessages) &&
          RecordingStudioMessages.respond_to?(:configuration) &&
          RecordingStudioMessages.configuration.respond_to?(:hooks)
      end
