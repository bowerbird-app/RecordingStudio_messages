...
module RecordingStudioMessages
  module Generators
    class InstallGenerator < Rails::Generators::Base
      ...
      desc "Installs RecordingStudioMessages engine into your application"

      class_option(
        :mount_path,
        type: :string,
        default: "/recording_studio_messages",
        desc: "Route prefix used when mounting the engine"
      )

      def mount_engine
        route %(mount RecordingStudioMessages::Engine, at: "#{options[:mount_path]}")
      end

      def copy_initializer
        template "recording_studio_messages_initializer.rb", "config/initializers/recording_studio_messages.rb"
      end

      def add_yaml_config
        return unless yes?("Would you like to add `config/recording_studio_messages.yml` for environment-specific settings? [y/N]")

        template "recording_studio_messages.yml", "config/recording_studio_messages.yml"
      end
      ...
      def add_tailwind_source
        ...
        if missing_lines.empty?
          say "Tailwind already configured to include RecordingStudioMessages and FlatPack sources.", :green
          return
        end
      ...
      def inject_tailwind_sources(tailwind_css_path, missing_lines)
        ...
        say "Added RecordingStudioMessages and FlatPack sources to Tailwind CSS configuration.", :green
        say "Run 'bin/rails tailwindcss:build' to rebuild your CSS.", :green
      end

      def formatted_tailwind_source_block(missing_lines)
        [
          "\n/* Include RecordingStudioMessages engine views for Tailwind CSS */",
      ...
      def tailwind_source_lines
        [
          ...
          '@source "../../../../../../usr/local/bundle/ruby/**/bundler/gems/recording_studio_messages-*/app/views/**/*.erb";',
