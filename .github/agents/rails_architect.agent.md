...
**Engine Isolation**: Respect namespace isolation. All code should be properly namespaced under the engine's module (`RecordingStudioMessages`). Use `isolate_namespace` patterns correctly.
...
**Engine-Specific**:
- Prefix all database tables with the engine name (e.g., `recording_studio_messages_error_logs`)
...
- **Browser Simulation**: When modifying UI or flows (like `dashboard/index.html.erb`), explicitly verify the user experience:
  ...
  2. Navigate to the relevant page (e.g., `/recording_studio_messages` or `/recording_studio_messages/analytics`).
...
When generating code, include file paths in comments (e.g., `# app/models/recording_studio_messages/error_log.rb`).
...
**Configuration**:
- Provide a configuration block: `RecordingStudioMessages.configure do |config|`
