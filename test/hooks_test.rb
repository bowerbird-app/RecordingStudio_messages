...
class HooksTest < Minitest::Test
  def setup
    @hooks = RecordingStudioMessages::Hooks.new
  end
  ...
  def test_raise_on_error_raises_hook_error
    ...
    assert_raises(RecordingStudioMessages::Hooks::HookError) do
      @hooks.run(:after_initialize)
    end
  end
  ...
  def test_class_run_delegates_to_configuration
    RecordingStudioMessages.configuration.hooks
    called = false
  ...
  def test_class_trigger_is_alias_for_run
    called = false
    RecordingStudioMessages.configuration.hooks.on(:custom_event) { called = true }

    RecordingStudioMessages::Hooks.trigger(:custom_event)
  ...
  def test_class_run_around_delegates_to_configuration
    events = []
    RecordingStudioMessages.configuration.hooks.around_service do |_context, block|
      events << :around
      block.call
    end

    result = RecordingStudioMessages::Hooks.run_around(:around_service, :service) do
      events << :core
      :ok
    end
