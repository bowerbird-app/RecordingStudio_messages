...
#    - No old recording_studio_messages references in Ruby files
#    - No old directories (lib/recording_studio_messages, app/controllers/recording_studio_messages, etc.)
...
class RenameVerificationTest < Minitest::Test
  ...
  # These tests check that the original "recording_studio_messages" name has been
  # completely replaced. They only run after a rename has occurred.

  def test_no_old_recording_studio_messages_references_in_ruby_files
    # Skip if current name IS recording_studio_messages (nothing to check - hasn't been renamed yet)
    skip if @gem_name == "recording_studio_messages"
    ...
    ruby_files.each do |file|
      ...
      files_with_old_refs << file if content.include?("recording_studio_messages") || content.include?("RecordingStudioMessages")
    end

    assert files_with_old_refs.empty?,
           "Found old 'recording_studio_messages' references in:\n#{files_with_old_refs.join("\n")}"
  end

  def test_no_old_recording_studio_messages_directories
    skip if @gem_name == "recording_studio_messages"

    old_dirs = [
      File.join(@root, "lib", "recording_studio_messages"),
      File.join(@root, "app", "controllers", "recording_studio_messages"),
      File.join(@root, "app", "views", "recording_studio_messages")
    ]

    existing_old_dirs = old_dirs.select { |d| Dir.exist?(d) }

    assert existing_old_dirs.empty?,
           "Found old 'recording_studio_messages' directories:\n#{existing_old_dirs.join("\n")}"
  end

  def test_no_old_gemspec_file
    skip if @gem_name == "recording_studio_messages"

    old_gemspec = File.join(@root, "recording_studio_messages.gemspec")
  ...
  def test_no_old_main_lib_file
    ...
    old_lib = File.join(@root, "lib", "recording_studio_messages.rb")
